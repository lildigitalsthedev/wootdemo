...
export type MeProfile = {
  ...
  handle: string;
  dm: string;
  avatar: string; // initials fallback shown when there's no avatarUrl
  avatarUrl?: string; // set to an image URL/data URL to show a real profile picture
  color: string;
  role: string;
  verified: boolean;
  business: string;
...
// Anything that reads the profile (nav bar avatar, profile page, settings,
// story rail, etc.) should use the `useMe()` hook below instead of a static
// import, so that changing the picture anywhere updates it everywhere else
// instantly, including the bottom nav / sidebar profile icon.
let meState: MeProfile = { ...INITIAL_ME };
const meListeners = new Set<() => void>();

function meSubscribe(listener: () => void) {
...
/** Update one or more fields on the logged-in user's profile (e.g. avatarUrl after picking a new photo). */
export function updateMe(patch: Partial<MeProfile>) {
  meState = { ...meState, ...patch };
  meListeners.forEach((listener) => listener());
}

/** React hook: subscribe to the logged-in user's profile. Re-renders automatically when it changes. */
export function useMe(): MeProfile {
  return useSyncExternalStore(meSubscribe, meSnapshot, meSnapshot);
}

/**
 * Static snapshot for non-component code or first-paint defaults. Prefer
