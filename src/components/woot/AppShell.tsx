...
export function AppShell({
  ...
  return (
    <div className="flex min-h-[100dvh] w-full flex-1 flex-col bg-background md:min-h-0">
      <header className="sticky top-0 z-20 grid shrink-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b bg-background/85 px-4 py-3 backdrop-blur-xl md:bg-background md:px-6 md:backdrop-blur-none">
      ...
      <main className={`flex min-h-0 min-w-0 flex-1 flex-col ${noPadX ? "" : "md:px-4 md:overflow-y-auto"}`}>{children}</main>
    </div>
  );
}
