...
const PLANS: Plan[] = [
  {
    ...
    features: [
      ...
      "Business username", "Basic business profile", "Reviews & ratings",
      "Stories", "Voice & video calls", "QR Code", "Shareable business link",
      "Basic analytics", "Warning banner until business review", "1 Admin", "AI Support",
    ],
  },
  {
...
function useRegionCurrency() {
  ...
  useEffect(() => {
    try {
      ...
      if (tz === "Africa/Lagos") setAuto("NGN");
    } catch { /* ignore */ }
    try {
      const locales = navigator.languages?.length ? navigator.languages : [navigator.language];
      if (locales.some((l) => l?.toUpperCase().endsWith("-NG"))) setAuto("NGN");
    } catch { /* ignore */ }
  }, []);

  return { currency: override ?? auto, setOverride };
}
...
export function PremiumSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [planOpen, setPlanOpen] = useState<string | null>("medium");
  const { currency, setOverride } = useRegionCurrency();

  return (
    <Modal open={open} onClose={onClose} title="Woot Premium">
      <div className="px-4 pb-8 pt-6">
        <div className="text-center">
          ...
          <h2 className="mt-4 text-[24px] font-black tracking-tight">Do more with Premium</h2>
          <p className="mx-auto mt-2 max-w-md text-[13px] text-muted-foreground">Boosts, AI tools, and analytics designed for businesses that ship.</p>
              ...
              <motion.div layout key={p.id}
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    ...
                    <button onClick={() => setPlanOpen(isOpen ? null : p.id)}
