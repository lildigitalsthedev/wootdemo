...
/**
 * The actual conversation UI: header, message list, composer.
 * Used both by the standalone /chat/$id route (mobile, deep links)
 * and embedded as the right-hand pane of the desktop split view.
 *
 * `onBack` is only rendered when provided (mobile / narrow layouts) —
 * on desktop the list stays visible so there's nothing to "go back" to.
 */
export function ChatThread({ b, onBack }: { b: Business; onBack?: () => void }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-surface">
      <header className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b bg-background/85 px-3 py-3 backdrop-blur-xl">
        {onBack ? (
          <button onClick={onBack} className="grid h-10 w-10 place-items-center rounded-full hover:bg-accent">
            <ArrowLeft size={18} />
          </button>
        ) : (
          <div />
